...
<aside class="w-[30%] bg-white rounded-xl shadow-md flex flex-col overflow-hidden">
	...
	<div class="flex-1 flex flex-col overflow-hidden">
		...
		{:else}
			<div class="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 hover:scrollbar-thumb-gray-400">
				{#each cameras as camera (camera.id)}
					<button
						class="flex gap-1 p-3 rounded-lg border-2 mb-3 cursor-pointer transition-all duration-200 text-left w-full hover:bg-gray-50 hover:border-gray-300 hover:translate-x-0.5"
					...
					>
						...
						<div class="flex-1 min-w-0">
							...
							<!-- <div class="text-sm text-gray-600 mb-0.5 flex items-center gap-1">
								...
								{@html highlightText(camera.time, searchName)}
							</div> -->
						</div>
					</button>
				{/each}
			...
			{#if pagination.totalPages > 1}
				<div class="flex justify-between items-start px-5 py-4 border-t border-gray-200 bg-gray-50">
					<button
