...
func NewServer(version, buildTag, runEnv string) (server *Server, err error) {
	...
	logDbConn, err = datasources.ConnectDb(datasources.DbConfig{
		DbDriver: "sqlite",
	...
	// 		DbDriver: "postgres",
	// 		DbName:   viper.GetString("db.postgres.db_name"),
	// 		Host:     viper.GetString("db.postgres_log.host"),
	// 		Username: viper.GetString("db.postgres_log.username"),
	// 		Password: viper.GetString("db.postgres_log.password"),
	// 		Port:     viper.GetInt("db.postgres_log.port"),
	// 		Timezone: "Asia/Bangkok",
	// 	})
	// 	if err != nil {
	// 		return
...
func connectToRedis() (redisStorage *redis.Storage, err error) {
	store := redis.New(redis.Config{
		...
		Database:  viper.GetInt("db.redis.db_name"),
		Reset:     false,
		TLSConfig: nil,
		PoolSize:  10,
	})
	if cmd := store.Conn().Ping(context.Background()); cmd.Err() != nil {
